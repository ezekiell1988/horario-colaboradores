...
const TOUR_STEPS: DriveStep[] = [
  {
    popover: {
      title: "Vista del día",
      description: "Muestra todos los colaboradores activos organizados por turno (Mañana, Tarde, Noche) y los que tienen día libre.",
    },
  },
  {
    element: "#coord-hoy-cards",
    popover: {
      title: "Tarjetas por turno",
      description: "Cada tarjeta lista los colaboradores en ese turno junto con su grupo. Los marcados con excepción aparecen atenuados.",
      side: "top",
    },
  },
];
...
export default function CoordinadorHoyPage() {
  ...
  useEffect(() => {
    fetch("/api/roll/hoy")
      .then((r) => {
        ...
        return r.json() as Promise<HoyData>;
      })
      .then(setData)
      .catch(() => showToast("Error al cargar el roll de hoy"))
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-bold text-gray-900 capitalize">{formatFecha(data.fecha)}</h1>
      ...
      <div id="coord-hoy-cards" className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        {(["T1", "T2", "T3"] as const).map((turno) => {
          ...
          return (
            <div key={turno} className={`rounded-xl border ${cfg.border} ${cfg.bg} overflow-hidden`}>
          ...
          <div className="rounded-xl border border-gray-200 bg-gray-50 overflow-hidden">
            <div className="px-4 py-3 bg-gray-100 text-gray-700">
              ...
              <div className="text-xs mt-0.5 opacity-60">
                {data.libre.length} colaborador{data.libre.length !== 1 ? "es" : ""}
              </div>
            </div>
            <ul className="divide-y divide-white/60 px-4 py-2">
              {data.libre.map((c) => (
